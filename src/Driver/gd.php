 ...
 * fonts.
 *
 * The options of this driver are configured in {@link \Ezc\Graph\Options\DriverOptions}
 * extending the basic driver options class {@link \Ezc\Graph\Options\DriverOptions}.
 *
 * <code>
 ...
 * @mainclass
 */
class ezcGraphGdDriver extends AbstractDriver
{
    ...
    {
        Base::checkDependency( 'Graph', Base::DEP_PHP_EXTENSION, 'gd' );
        $this->options = new GdDriverOptions( $options );
    }
    ...
    protected function getTextBoundings( $size, FontOptions $font, $text )
    {
        switch ( $font->type )
        {
            ...
            case ezcGraph::TTF_FONT:
                switch ( true )
                {
                    case BaseFeatures::hasFunction( 'imageftbbox' ) && !$this->options->forceNativeTTF:
                    ...
                    case BaseFeatures::hasFunction( 'imagettfbbox' ):
     ...
     * @return void
     */
    protected function renderText( $image, $text, $type, $path, Color $color, Coordinate $position, $size, ezcGraphRotation $rotation = null )
    {
        if ( $rotation !== null )
        {
        ...
        switch ( $type )
        {
            ...
            case ezcGraph::TTF_FONT:
                switch ( true )
                {
                    case BaseFeatures::hasFunction( 'imagefttext' ) && !$this->options->forceNativeTTF:
                        imageFtText(
                            $image, 
                            $size,
                            ( $rotation === null ? 0 : -$rotation->getRotation() ),
                        ...
                        break;
                    case BaseFeatures::hasFunction( 'imagettftext' ):
                        imageTtfText(
                            $image, 
                            $size,
                            ( $rotation === null ? 0 : -$rotation->getRotation() ),
     ...
     * @return void
     */
    protected function drawCircularArcStep( Coordinate $center, $width, $height, $size, $startAngle, $endAngle, Color $color )
    {
        $this->drawPolygon(
            array(
                new Coordinate(
