 ...
 * class {@link \Ezc\Graph\Options\FlashDriverOptions} extending the basic
 ...
 * @mainclass
 */
class ezcGraphFlashDriver extends AbstractDriver
{
    /**
     * Flash movie
     * 
    ...
    public function __construct( array $options = array() )
    {
        ...
        $this->options = new FlashDriverOptions( $options );
    }
     ...
     * @return void
     */
    protected function setShapeColor( SWFShape $shape, Color $color, $thickness, $filled )
    {
        if ( $filled )
        {
            switch ( true )
    ...
    protected function getTextBoundings( $size, FontOptions $font, $text )
    {
        ...
        $boundings = new Boundings( 0, 0, $t->getWidth( $text ), $size );
     ...
     * @return void
     */
    protected function renderText( $id, $text, $chars, $type, $path, Color $color, Coordinate $position, $size, $rotation = null )
    {
        $movie = $this->getDocument();
