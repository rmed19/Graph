...
abstract class ezcGraphPalette
{
     ...
     * 
     * @param mixed $color Color to transform into a Color object
     * @return Color
     */
    ...
    public function __set( $propertyName, $propertyValue )
    {
        switch ( $propertyName )
        {
            ...
            case 'elementBackground':
            case 'elementBorderColor':
                $this->$propertyName = Color::create( $propertyValue );
                break;

            case 'dataSetColor':
                if ( !is_array( $propertyValue ) )
                {
                    throw new ezcBaseValueException( $propertyName, $propertyValue, 'array( Color )' );
                }

                $this->dataSetColor = array();
                foreach ( $propertyValue as $value )
                {
                    $this->dataSetColor[] = Color::create( $value );
                }
                $this->colorIndex = -1;
                break;
            case 'dataSetSymbol':
