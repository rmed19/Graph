...
use Ezc\Graph\DataContainer\ChartDataContainer;
 ...
 * @property AbstractDriver $driver
...
abstract class ezcGraphChart
{
     ...
     * 
     * @var array(\Ezc\Graph\Interfaces\AbstractChartElement)
     */
    ...
    {
        $this->palette = new Tango();
        $this->data = new ChartDataContainer( $this );

        // Add standard elements
        $this->addElement( 'background', new ChartElementBackground() );
        $this->elements['background']->position = ezcGraph::CENTER | ezcGraph::MIDDLE;

        $this->addElement( 'title', new ChartElementText() );
        $this->elements['title']->position = ezcGraph::TOP;
        $this->renderElement['title'] = false;

        $this->addElement( 'subtitle', new ChartElementText() );
        $this->elements['subtitle']->position = ezcGraph::TOP;
        $this->renderElement['subtitle'] = false;

        $this->addElement( 'legend', new ChartElementLegend() );
        ...
        $this->properties['renderer'] = new Renderer2d();
     ...
     * @param AbstractChartElement $element Chart element
     * @return void
     */
    protected function addElement( $name, AbstractChartElement $element )
    {
    ...
    public function __set( $propertyName, $propertyValue ) 
    {
        switch ( $propertyName ) {
            ...
            case 'driver':
                ...
                {
                    throw new ezcBaseValueException( $propertyName, $propertyValue, 'AbstractDriver' );
                }
                break;
            case 'palette':
            ...
            case 'renderedFile':
                ...
                break;
            case 'options':
                if ( $propertyValue instanceof ChartOptions )
                {
