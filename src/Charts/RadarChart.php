...
use Ezc\Graph\Renderer\AxisRadarLabelRenderer;
use Ezc\Graph\Structs\Context;
use Ezc\Graph\Structs\Coordinate;

/**
 * File containing the RadarChart class
 *
 ...
 *  - axis ( {@link Ezc\Graph\Axis\ChartElementNumericAxis} )
 *  - ratation axis ( {@link Ezc\Graph\Axis\ChartElementLabeledAxis} )
 *
 ...
 * {@link \Ezc\Graph\Options\RadarChartOptions} extending the
 ...
 * </code>
 *
 * @property RadarChartOptions $options
 *           Chart options class
 *
 * @version //autogentag//
 * @package Graph
 * @mainclass
 */
class RadarChart extends ezcGraphChart
{
    /**
    ...
    {
        $this->options = new RadarChartOptions( $options );
        $this->options->highlightFont = $this->options->font;

        parent::__construct();

        $this->elements['rotationAxis'] = new ChartElementLabeledAxis();

        $this->addElement( 'axis', new ChartElementNumericAxis() );
        $this->elements['axis']->position = ezcGraph::BOTTOM;
        $this->elements['axis']->axisLabelRenderer = new AxisRadarLabelRenderer();
        $this->elements['axis']->axisLabelRenderer->outerStep = true;

        $this->addElement( 'rotationAxis', new ChartElementLabeledAxis() );
    ...
    public function __set( $propertyName, $propertyValue ) 
    {
        switch ( $propertyName ) {
            case 'axis':
                if ( $propertyValue instanceof AbstractChartElementAxis )
                {
                ...
                {
                    throw new ezcBaseValueException( $propertyName, $propertyValue, 'AbstractChartElementAxis' );
                }
                break;
            case 'rotationAxis':
                if ( $propertyValue instanceof AbstractChartElementAxis )
     ...
     * @param Boundings $boundings 
     * @param Coordinate $center 
     * @param float $position 
     * @param float $lastPosition 
     * @return void
     */
    protected function drawRotatedAxis( AbstractChartElementAxis $axis, Boundings $boundings, Coordinate $center, $position, $lastPosition = null )
    {
        ...
        if ( $axis->axisLabelRenderer instanceof AxisRadarLabelRenderer )
        {
     ...
     * @param Boundings $boundings Remaining boundings
     * @return void
     */
    protected function renderData( ezcGraphRenderer $renderer, Boundings $boundings )
    {
        // Apply axis space
        ...
        foreach ( $this->data as $datasetName => $data )
        {
            ...
            foreach ( $data as $key => $value )
            {
                ...
                $renderer->drawRadarDataLine(
                    $boundings,
                    new Context( $datasetName, $key, $data->url[$key] ),
                    $data->color->default,
                    clone $center,
