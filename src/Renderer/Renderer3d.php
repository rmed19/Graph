...
use Ezc\Graph\Interfaces\StackedBarsRendererInterface;
 ...
 *
 * The class options are defined in the class {@link \Ezc\Graph\Options\Renderer3dOptions}
        ...
        StackedBarsRendererInterface
{
     ...
     *          'context' => Context // context of call
    ...
    {
        $this->options = new Renderer3dOptions( $options );
    }
     ...
     */
    protected function get3dCoordinate( Coordinate $c, $front = 1. )
    {
     ...
     * @param Color $color Color of pie segment
        ...
        $moveOut = false )
    {
        ...
        if ( $label )
        {
            ...
            $direction = ( $endAngle + $startAngle ) / 2;
            $pieSegmentCenter = new Coordinate(
            ...
            $this->barPostProcessing[] = array(
                ...
                'parameters' => array(
                    ...
                    new LinearGradient(
                        ...
                        Color::fromHex( '#FFFFFF' )->transparent( 1 - $this->options->barChartGleam )
                    ),
                    true
                ),
            );
        ...
        $endDepth )
    {
        ...
        $this->barPostProcessing[] = array(
            ...
            'parameters' => array(
                array(
                    $this->get3dCoordinate( new Coordinate( $barCoordinateArray['x'][0], $barCoordinateArray['y'][0] ), $midDepth ),
                    $this->get3dCoordinate( new Coordinate( $barCoordinateArray['x'][0], $barCoordinateArray['y'][1] ), $midDepth ),
                    $this->get3dCoordinate( new Coordinate( $barCoordinateArray['x'][1], $barCoordinateArray['y'][1] ), $startDepth ),
                    $this->get3dCoordinate( new Coordinate( $barCoordinateArray['x'][1], $barCoordinateArray['y'][0] ), $startDepth ),
                ),
                $color,
                true
            ),
        ...
        $this->barPostProcessing[] = array(
            ...
            'parameters' => array(
                array(
                    $this->get3dCoordinate( new Coordinate( $barCoordinateArray['x'][2], $barCoordinateArray['y'][0] ), $midDepth ),
                    $this->get3dCoordinate( new Coordinate( $barCoordinateArray['x'][2], $barCoordinateArray['y'][1] ), $midDepth ),
                    $this->get3dCoordinate( new Coordinate( $barCoordinateArray['x'][1], $barCoordinateArray['y'][1] ), $startDepth ),
                    $this->get3dCoordinate( new Coordinate( $barCoordinateArray['x'][1], $barCoordinateArray['y'][0] ), $startDepth ),
                ),
                $color->darken( $this->options->barDarkenSide ),
        ...
        $this->barPostProcessing[] = array(
            ...
            'parameters' => array(
                array(
                    $this->get3dCoordinate( new Coordinate( $barCoordinateArray['x'][1], $topLocation ), $startDepth ),
                    $this->get3dCoordinate( new Coordinate( $barCoordinateArray['x'][2], $topLocation ), $midDepth ),
                    $this->get3dCoordinate( new Coordinate( $barCoordinateArray['x'][3], $topLocation ), $endDepth ),
                    $this->get3dCoordinate( new Coordinate( $barCoordinateArray['x'][0], $topLocation ), $midDepth ),
                ),
                $color->darken( $this->options->barDarkenTop ),
                true
            ),
        ...
        $symbol )
    {
        ...
        $barCenterBottom = new Coordinate(
            $this->dataBoundings->x0 + $this->xAxisSpace + $position->x * ( $this->dataBoundings->x1 - ( $this->dataBoundings->x0 + 2 * $this->xAxisSpace ) ) + $offset + $barWidth / 2,
            $this->dataBoundings->y0 + $this->yAxisSpace + $axisPosition * ( $this->dataBoundings->y1 - ( $this->dataBoundings->y0 + 2 * $this->yAxisSpace ) )
        );
        ...
        $thickness = 1. )
    {
        ...
        $startAxisCoord = new Coordinate( 
            $this->dataBoundings->x0 + $this->xAxisSpace + $start->x * ( $this->dataBoundings->x1 - ( $this->dataBoundings->x0 + 2 * $this->xAxisSpace ) ),
            $this->dataBoundings->y0 + $this->yAxisSpace + $axisPosition * ( $this->dataBoundings->y1 - ( $this->dataBoundings->y0 + 2 * $this->yAxisSpace ) )
        );
        $endAxisCoord = new Coordinate( 
            $this->dataBoundings->x0 + $this->xAxisSpace + $end->x * ( $this->dataBoundings->x1 - ( $this->dataBoundings->x0 + 2 * $this->xAxisSpace ) ),
            $this->dataBoundings->y0 + $this->yAxisSpace + $axisPosition * ( $this->dataBoundings->y1 - ( $this->dataBoundings->y0 + 2 * $this->yAxisSpace ) )
        );
     ...
     * @param int $dataNumber Number of dataset
     * @param int $dataCount Count of datasets in chart
     * @param FontOptions $font Font used for highlight string
     * @param string $text Acutual value
     * @param int $size Size of highlight text
     * @param Color $markLines
     * @param int $xOffset
     * @param int $yOffset
        ...
        $type = ezcGraph::LINE )
    {
        ...
        if ( $end->y < $axisPosition )
        {
            $this->driver->drawTextBox(
                $text,
                $this->get3dCoordinate( new Coordinate(
                    $dataPoint->x - $width / 2,
        ...
        $type = ezcGraph::VERTICAL )
    {
        ...
        // Draw all labels
        $labelPosition = new Coordinate( $boundings->x0, $boundings->y0 );
        ...
        $titleSize = 16 )
    {
        ...
        if ( $background instanceof Color )
        {
            ...
            $this->driver->drawPolygon(
                array(
                    new Coordinate( $boundings->x0, $boundings->y0 ),
                    new Coordinate( $boundings->x1, $boundings->y0 ),
                    new Coordinate( $boundings->x1, $boundings->y1 ),
                    new Coordinate( $boundings->x0, $boundings->y1 ),
                ),
                $background,
                true
            );
        ...
        if ( $title !== false )
        {
            switch ( $this->options->titlePosition )
            {
                ...
                case ezcGraph::BOTTOM:
                    $this->driver->drawTextBox(
                        $title,
                        new Coordinate( $boundings->x0, $boundings->y1 - $titleSize ),
                        $boundings->x1 - $boundings->x0,
                        $titleSize,
                        $this->options->titleAlignement
                    );
     ...
     * @param Color $color Color of the grid line
     * @return void
     */
    public function drawGridLine( Coordinate $start, Coordinate $end, Color $color )
    {
     ...
     * @return void
     */
    public function drawStepLine( Coordinate $start, Coordinate $end, Color $color )
    {
     ...
     * @param Coordinate $start Start point of axis
     * @param Coordinate $end Endpoint of axis
     * @param AbstractChartElementAxis $axis Axis to render
     * @param AbstractAxisLabelRenderer $labelClass Used label renderer
     * @return void
     */
    public function drawAxis(
        ...
        AbstractAxisLabelRenderer $labelClass = null )
    {
