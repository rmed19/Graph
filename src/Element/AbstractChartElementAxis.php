<?php
/**
 * File containing the abstract AbstractChartElementAxis class
 *
 ...
 * ChartElementDateAxis. The currently available axis types are:
 *
 * - Ezc\Graph\Axis\ChartElementDateAxis
 * - Ezc\Graph\Axis\ChartElementLabeledAxis
 * - Ezc\Graph\Axis\ChartElementLogarithmicalAxis
 ...
 * calcualted and rendered by the AbstractAxisLabelRenderer classes. You can
 ...
 * 
 * - Ezc\Graph\Renderer\AxisBoxedLabelRenderer
 *
 *   Renders grid and labels like commonly used in bar charts, with the label
 *   between two grid lines.
 *  
 * - Ezc\Graph\Renderer\AxisCenteredLabelRenderer
 *
 *   Centers the label right next to a tick. Commonly used for labeled axis.
 *
 * - Ezc\Graph\Renderer\AxisExactLabelRenderer
 *
 *   Put the label next to each tick. Commonly used for numeric axis.
 *
 * - Ezc\Graph\Renderer\AxisNoLabelRenderer
 *
 *   Renders no labels.
 *
 * - Ezc\Graph\Renderer\AxisRadarLabelRenderer
 *
 *   Special label renderer for radar charts.
 *
 * - Ezc\Graph\Renderer\AxisRotatedLabelRenderer
 *
 ...
 * @property Color $majorGrid
 *           Color of major majorGrid.
 * @property Color $minorGrid
 ...
 * @property AbstractAxisLabelRenderer $axisLabelRenderer
 ...
 * @package Graph
 */
abstract class AbstractChartElementAxis extends AbstractChartElement
{
    /**
     * Axis label renderer class
     * 
     * @var AbstractAxisLabelRenderer
     */
    ...
    public function __construct( array $options = array() )
    {
        ...
        {
            $this->axisLabelRenderer = new AxisExactLabelRenderer();
        }
    }
    ...
    public function __set( $propertyName, $propertyValue )
    {
        switch ( $propertyName )
        {
            ...
            case 'outerAxisSpace':
                ...
                break;
            case 'majorGrid':
                $this->properties['majorGrid'] = Color::create( $propertyValue );
                break;
            case 'minorGrid':
                $this->properties['minorGrid'] = Color::create( $propertyValue );
                break;
            case 'majorStep':
                if ( !is_numeric( $propertyValue ) ||
                     ( $propertyValue <= 0 ) )
            ...
            case 'axisLabelRenderer':
                if ( $propertyValue instanceof AbstractAxisLabelRenderer )
                {
                    $this->axisLabelRenderer = $propertyValue;
                }
                else
                {
                    throw new ezcBaseValueException( $propertyName, $propertyValue, 'AbstractAxisLabelRenderer' );
                }
                break;
            case 'labelCallback':
     ...
     */
    public function render( ezcGraphRenderer $renderer, Boundings $boundings, Boundings $innerBoundings = null )
    {
